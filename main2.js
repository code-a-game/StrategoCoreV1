...
const GAME_TRANSITION_TIME = 5000; // 5 seconds in milliseconds
...
const CHARACTER_DEFINITIONS = [
    { rank: -1, name: "Core Command", id: "F", count: 1, isCoreCommand: true },
    { rank: 10, name: "Star Commander", id: "10", count: 1, isStarCommand: true },
    ...
    { rank: 3, name: "Engineer", id: "3", count: 5, isEngineer: true }, // Special ability
    { rank: 2, name: "Power Glider", id: "2", count: 8 },
    { rank: 1, name: "Stealth Squad", id: "S", count: 1, isStealthSquad: true }, // Special ability
    { rank: 0, name: "Recon Drone", id: "D", count: 6, isReconDrone: true }, // Special rank 0 for Bomb
];
...
//        "wss://p5js-spaceman-server-29f6636dfb6c.herokuapp.com",
    ...
    me = partyLoadMyShared({
        playerNumber: Math.floor(Math.random() * 999999) + 1,
        playerName: "",
        team: null,
        characterId: null,
        characterRank: null,
...
function draw() {
    ...
    allPlayers = guests.filter(p => p.isReady && (p.team === 'blue' || p.team === 'green')) //[me, ...guests];

    //console.log(guests) jens
    // Host-specific logic
    if (partyIsHost()) {
        handleHostDuties();
    }
...
function drawGameStats() {
    ...
    let blueTeamCount = allPlayers.filter(p => p.isReady && p.team === 'blue').length;
    let greenTeamCount = allPlayers.filter(p => p.isReady && p.team === 'green').length;
...
function drawCharacterList() {
    ...
    let myTeamFlagChosen = allPlayers.some(p => p.team === me.team && p.characterId === 'F' && p.hasCharacter);
...
function handlePlayerMovement() {
    ...
    if (keyIsDown(LEFT_ARROW) || keyIsDown(65)) dx -= speed; // Left or A
    if (keyIsDown(RIGHT_ARROW) || keyIsDown(68)) dx += speed; // Right or D
    if (keyIsDown(UP_ARROW) || keyIsDown(87)) dy -= speed; // Up or W
    if (keyIsDown(DOWN_ARROW) || keyIsDown(83)) dy += speed; // Down or S
    ...
    targetX = constrain(targetX, GAME_AREA_X + me.size / 2, GAME_AREA_RIGHT - me.size / 2);
    targetY = constrain(targetY, GAME_AREA_Y + me.size / 2, GAME_AREA_BOTTOM - me.size / 2);
...
function handleHostDuties() {
    ...
    switch (shared.gameState) {
        case "GAME-SETUP":
            handleGameSetupHost(currentTime);
            break;
        case "IN-GAME":
            if (!shared.coreCommandLost) {
                handleGameInProgressHost(currentTime);
            }
            break;
        case "GAME-FINISHED":
            handleGameFinishedHost(currentTime);
            break;
    }
}
...
function handleDisconnectedPlayers() {
    ...
    const connectedPlayerIds = new Set(allPlayers.map(p => p.playerNumber));
...
function handleGameSetupHost(currentTime) {
    ...
    if (conditionsMet && shared.gameStartTimerStartTime === null) {
        ...
        shared.gameStartTimerStartTime = currentTime;
    }
    ...
    // Start game when countdown finishes. jens1
    if (shared.gameStartTimerStartTime && currentTime - shared.gameStartTimerStartTime >= GAME_TRANSITION_TIME) {
    ...
    shared.gameStartTimerSeconds = Math.floor(GAME_TRANSITION_TIME / 1000) - Math.floor((currentTime - shared.gameStartTimerStartTime) / 1000)
}

function handleGameInProgressHost(currentTime) {
...
function detectCollisionsAndInitiateBattles() {
    ...
    for (let i = 0; i < activeCharacters.length; i++) {
        ...
        let player1 = allPlayers.find(p => p.playerNumber === char1.takenByPlayerId);
        ...
        for (let j = i + 1; j < activeCharacters.length; j++) {
            ...
            let player2 = allPlayers.find(p => p.playerNumber === char2.takenByPlayerId);
            ...
            if (d < (player1.size / 2 + player2.size / 2)) {
                ...
                shared.characterList[char1Index].battleOpponentInfo = { name: char2.name, rank: char2.rank, playerName: player2.playerName };
                ...
                // Include opponent player name
                shared.characterList[char2Index].battleOpponentInfo = { name: char1.name, rank: char1.rank, playerName: player1.playerName };
...
function resolveBattles(currentTime) {
    ...
    for (const charInBattle of charactersInBattle) {
        ...
        if (charInBattle.battleStartTime && (currentTime - charInBattle.battleStartTime >= BATTLE_RESOLUTION_TIME)) {
            readyToResolve = true;
...
function handleGameFinishedHost(currentTime) {
    ...
    if (shared.resetTimerStartTime === null && !shared.resetFlag) {
        ...
        shared.resetTimerStartTime = currentTime;
    }

    // Trigger reset when countdown finishes
    if (shared.resetTimerStartTime && currentTime - shared.resetTimerStartTime >= GAME_TRANSITION_TIME && !shared.resetFlag) {
        console.log("HOST: Reset timer finished. Setting reset flag.");
        shared.resetFlag = true;
        shared.resetTimerStartTime = null;
    }

    shared.resetTimerSeconds = Math.floor(GAME_TRANSITION_TIME / 1000) - Math.floor((currentTime - shared.resetTimerStartTime) / 1000)
